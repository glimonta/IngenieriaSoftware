...
DECLARE
   ...
   C2 CURSOR FOR SELECT C.CANTIDAD, E.TIPO_PLAN 
      FROM ESTA_AFILIADO E, TIENE T, CONTIENE C, ADICIONAL A
      WHERE NEW.ID = E.ID AND T.NOMBRE_PLAN = E.NOMBRE_PLAN AND
      T.TIPO_PLAN = E.TIPO_PLAN AND C.NOMBRE_PAQUETE = T.NOMBRE_PAQUETE AND
      C.NOMBRE_SERVICIO = NEW.NOMBRE_SERVICIO AND
      A.NOMBRE_SERVICIO = C.NOMBRE_SERVICIO AND
      A.TIPO_PLAN = E.TIPO_PLAN AND
      E.FECHA_INIC <= NEW.FECHA AND
      (E.FECHA_FIN IS NULL OR NEW.FECHA < E.FECHA_FIN) AND 
      T.FECHA_INIC <= NEW.FECHA AND (T.FECHA_FIN IS NULL OR 
      NEW.FECHA < T.FECHA_FIN);
      
      
   --C2 CURSOR FOR SELECT CANTIDAD_ADICIONAL 
     -- FROM ADICIONAL A WHERE NEW.ID = P.ID AND
